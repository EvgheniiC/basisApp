...
def get_bundle(route, tpl, ext, paths, type=False):
    if route and tpl and ext:
        return {"instance": Bundle(*paths, output=get_path(route, tpl, ext, type), filters=get_filter(ext)),
...
bundles = {
    "post": {
        "all": {
            "css": [
                get_bundle('post','all','css',['css/blocks/table.css','css/libs/font-awesome.miv.css'])
            ],
            "js":[
                get_bundle('post','all','js',['js/blocks/js1.js','js/blocks/js2.js','js/blocks/js2.js'])
            ]
        },
        "create": {},
        "update": {}
